...
from config import BASE_DIR, HISTORY_FILE, COOKIE_FILE, NETSCAPE_TEMP, BACKUP_FILENAMES
...
class BiliManager:
    ...
    def load_root_cookie_and_identify(self):
        if os.path.exists("last_login_cookie.json"):
            try:
                with open("last_login_cookie.json", 'r') as f:
                    c = json.load(f)
    ...
    def switch_user(self, uid):
        ...
        self.init_paths()
        self.load_data()
        with open("last_login_cookie.json", 'w') as f:
            json.dump(self.session.cookies.get_dict(), f)
        self.save_data()
    ...
    def get_netscape_cookie_path(self):
        try:
            ...
            with open(NETSCAPE_TEMP, 'w') as f: f.write(content)
            return NETSCAPE_TEMP
        except: return None

    def logout(self):
        self.session.cookies.clear()
        if os.path.exists("last_login_cookie.json"): os.remove("last_login_cookie.json")
        self.uid = "guest"
        self.init_paths()
        self.history = set()
